...
class ChebConvAttention(MessagePassing):
    ...
    def forward(
        ...
        """
        print(f"\t\t >> running ChebConvAttention forward")
        ...
        if lambda_max is None:
            print("lambda_max is None")
            lambda_max = torch.tensor(2.0, dtype=x.dtype, device=x.device)
        if not isinstance(lambda_max, torch.Tensor):
            print("not isinstance(lambda_max, torch.Tensor)")
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        print(f"\t\t >> running SpatialAttention forward")
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        print(f"\t\t >> running TemporalAttention forward")
        # lhs = left hand side embedding;
        # to calculcate it : 
...
class ASTGCNBlock(nn.Module):
    ...
    def forward(
        ...
        """
        print(f"\t\t >> running ASTGCNBlock forward")
        ...
        if not isinstance(edge_index, list):
            print("not isinstance(edge_index, list)")
            data = Data(
...
class ASTGCN(nn.Module):
    ...
    def __init__(
        ...
        print(f"\n\nUsing the modified version of ASTGCN\n\n")
    ...
    def forward(
        ...
        """
        print(f"\t\t >> running ASTGCN forward")
        ali_counter = 1
        for block in self._blocklist:
            print(f"\t\t >> running block {ali_counter}")
