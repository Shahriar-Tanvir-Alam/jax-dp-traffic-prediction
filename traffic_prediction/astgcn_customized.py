...
class ChebConvAttention(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if verbose: print(f"\t\t >> Creating ChebConvAttention")
        self.edge_index = edge_index
        self.lambda_max = None
        assert edge_index is not None, print("edge_index is None")
...
#         self._weight = Parameter(torch.Tensor(K, in_channels, out_channels)).to(edge_index.get_device())
        self._weight = Parameter(torch.Tensor(K, in_channels, out_channels)).to(edge_index.device)

        if bias:
#             self._bias = Parameter(torch.Tensor(out_channels)).to(edge_index.get_device())
            self._bias = Parameter(torch.Tensor(out_channels)).to(edge_index.device)
        else:
...
#         spatial_attention: torch.FloatTensor,
        ...
        """
        if verbose: print(f"\t\t >> running ChebConvAttention forward")
        if self._normalization != "sym" and self.lambda_max is None:
            raise ValueError(
        ...
        if self.lambda_max is None:
            self.lambda_max = torch.tensor(2.0, dtype=x.dtype, device=x.device)
        if not isinstance(lambda_max, torch.Tensor):
            self.lambda_max = torch.tensor(self.lambda_max, dtype=x.dtype, device=x.device)
        ...
        Att_norm = norm * self.spatial_attention[:, row, col] # spatial_attention for example (32, 307, 307), -> (954) * (32, 954) -> (32, 954)
        num_nodes = x.size(self.node_dim) #for example 307
        # (307, 307) * (32, 307, 307) -> (32, 307, 307) -permute-> (32, 307,307) * (32, 307, 1) -> (32, 307, 1)
        TAx_0 = torch.matmul(
            (torch.eye(num_nodes).to(edge_index.device) * self.spatial_attention).permute(
                0, 2, 1
            ),
            x,
        ) #for example (32, 307, 1)
        if verbose: print(f"device of self._weight[0]: {self._weight[0].get_device()}")
        if verbose: print(f"device of TAx_0: {TAx_0.get_device()}")
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        if verbose: print(f"\t\t >> running SpatialAttention forward")
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, X: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        if verbose: print(f"\t\t >> running TemporalAttention forward")
        # lhs = left hand side embedding;
        # to calculcate it : 
...
class ASTGCNBlock(nn.Module):
    ...
    def __init__(
        ...
        if verbose: print(f"\t\t >> Creating ASTGCNBlock")
        self.edge_index = edge_index
        assert edge_index is not None, print("edge_index is None")
...
#         )
        ...
        for t in range(num_of_timesteps):
            self._chebconv_attention[t] = ChebConvAttention(
...
#         edge_index: Union[torch.LongTensor, List[torch.LongTensor]],
        ...
        """
        if verbose: print(f"\t\t >> running ASTGCNBlock forward")
        ...
        if not isinstance(self.edge_index, list):
            if verbose: print("not isinstance(edge_index, list)")
            data = Data(
                edge_index=self.edge_index, edge_attr=None, num_nodes=num_of_vertices
            )
            ...
            X_hat = []
            for t in range(num_of_timesteps):
                self._chebconv_attention[t].lambda_max = lambda_max
                self._chebconv_attention[t].spatial_attention = X_tilde
                X_hat.append(
                    torch.unsqueeze(
                        self._chebconv_attention[t](
#                             X[:, :, :, t], self.edge_index, X_tilde, lambda_max=lambda_max
                            X[:, :, :, t],
                        ),
                        -1,
                    )
            ...
            assert 1==2, print("Check again!")
#         else:
#             print("isinstance(edge_index, list)")
#             X_hat = []
#             for t in range(num_of_timesteps):
#                 data = Data(
#                     edge_index=self.edge_index[t], edge_attr=None, num_nodes=num_of_vertices
#                 )
#                 if self._normalization != "sym":
#                     lambda_max = LaplacianLambdaMax()(data).lambda_max
#                 else:
...
#                     torch.unsqueeze(
#                         self._chebconv_attention[t](
#                             X[:, :, :, t], self.edge_index[t], X_tilde, lambda_max=lambda_max
#                         ),
#                         -1,
#                     )
#                 )
...
class Customized_ASTGCN(nn.Module):
    ...
    def __init__(
        ...
        super(Customized_ASTGCN, self).__init__()
        if verbose: print(f"\n\nUsing the modified ASTGCN version 1\n\n")
        assert edge_index is not None, print("edge_index is None")
...
#         edge_index: torch.LongTensor
        ...
        """
        if verbose: print(f"\t\t >> running ASTGCN forward")
        ali_counter = 1
        for block in self._blocklist:
            if verbose: print(f"\t\t >> running block {ali_counter}")
...
#             X = block(X, edge_index) 
        ...
        if verbose: print(f"\t\t >> running _final_conv")
