...
def main():
    ...
    config_path = "data/m_2_3_y_2019_optimals/config_MP0_o_optimal_uber_eps1.yaml"
    ...
    train_x_torch = train_data_list[4][0]
    train_y_torch = train_data_list[2][1]

    train_x = jnp.array(train_x_torch.detach().cpu().numpy())
    train_y = jnp.array(train_y_torch.detach().cpu().numpy())

    val_x, _ = dataloader_to_jax(val_loader_list[4])
    _, val_y = dataloader_to_jax(val_loader_list[2])
    test_x, _ = dataloader_to_jax(test_loader_list[4])
    _, test_y = dataloader_to_jax(test_loader_list[2])
    ...
    print("\nJAX MP0_o private DP-SGD training with RDP accountant finished successfully.")
